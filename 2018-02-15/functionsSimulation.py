...
def calculateForcesEnergy(atomListXYZNow, bondList, angleList, dihedralList, typeList, moleculeNumberList, boxSize, rLJCutOff): # add dihedralList
    ...
    for i in range(0, pairwiseDistances.shape[0]):
        pairwiseDistances[i,:] = floorDistanceVector(atomListXYZNow[i, :], atomListXYZNow, boxSize)
    ...
    LJList = np.concatenate((LJList[0].reshape(len(LJList[0]), 1), LJList[1].reshape(len(LJList[1]), 1)), axis=1)
