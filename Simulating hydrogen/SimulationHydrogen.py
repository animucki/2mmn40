...
from functionsHydrogenSimulation import *
...
######### READ "HYDROGEN" WHERE "WATER" IS STATED !!!!!!!!!!! #####################
...
# Set time parameters:
dt = 0.2 # Timestep for numerical integrator in fs
totalTime = 20 # Total simulation time in fs

# Set properties for the system:
boxSize = 265 # in Angström: box will have a volume of boxSize^3
...
rescale = 0 # in {0,1}: 1 means: do a rescale (i.e. use a thermostat) every time step. 0 means: no rescaling
...
myFileName = myIntegrator + " with numMolecules " + str(totalNumMolecules) + " time " + str(totalTime) + " fs dt " + str(dt) + " fs box " + str(boxSize/10) + " nm.xyz"
...
codeTimer = time.time()
# Get LJ parameters:
LJsigma = getLennartJonesCrossSigma(allTypes, allMoleculeNumbers)
LJsigma6 = LJsigma**6
LJsigma12 = LJsigma6**2
LJepsilon = getLennartJonesCrossEpsilon(allTypes, allMoleculeNumbers)
print("The LJ matrices took " + str(time.time() - codeTimer) + " seconds.")

# Start timer
myTime = time.time()

# Calculate the forces and energies at t = 0
forces, potentialEnergy[0] = calculateForcesEnergy(XYZ, allBonds, allAngles, allDihedrals, allTypes, allMoleculeNumbers, boxSize, rLJCutOff, LJsigma, LJepsilon, LJsigma6, LJsigma12)
...
for currentTime in np.arange(dt, totalTime+dt, dt):
    ...
    if myIntegrator == "euler":
        ...
        forces, potentialEnergy[index] = calculateForcesEnergy(XYZ, allBonds, allAngles, allDihedrals, allTypes, allMoleculeNumbers, boxSize, rLJCutOff, LJsigma, LJepsilon, LJsigma6, LJsigma12)
...
myFileNameTempEnergy = "T and E with "+myIntegrator+"numMolecules " + str(totalNumMolecules) + " time " + str(totalTime) + " fs dt " + str(dt) + " fs box " + str(boxSize/10) + " nm.csv"
