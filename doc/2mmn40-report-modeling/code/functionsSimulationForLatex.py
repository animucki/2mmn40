...
def getMoleculeStructure():
    ...
    struc =  {"Water": {"atoms": [["O"], ["H"], ["H"]], "masses": np.array([15.9994, 1.0080, 1.0080]), "bonds": np.array([[0, 1], [0, 2]]), "angles": np.array([[1, 0, 2]]), "dihedrals": np.empty([0, 4]), "kb": np.array([[5024.16], [5024.16]]), "r0": np.array([[0.9572], [0.9572]]), "ktheta": np.array([[100*6.2802]]), "theta0": np.array([[104.52*np.pi/180]]), "C1": np.empty([0, 1]), "C2": np.empty([0, 1]), "C3": np.empty([0, 1]), "C4": np.empty([0, 1]), "epsilon": [0.66386, 0, 0], "sigma": [3.15061, 0, 0]}, 
             "Hydrogen": {"atoms": [["H"], ["H"]], "masses": np.array([1.0080, 1.0080]), "bonds": np.array([[0, 1]]), "angles": np.empty([0, 3]), "dihedrals": np.empty([0, 4]), "kb": np.array([[1000]]), "r0": np.array([[1]]), "ktheta": np.empty([0, 1]), "theta0": np.empty([0, 1]), "C1": np.empty([0, 1]), "C2": np.empty([0, 1]), "C3": np.empty([0, 1]), "C4": np.empty([0, 1]), "epsilon": [0.125520, 0.125520], "sigma": [2.5, 2.5]},  
...
# Define a matrix of all sigmas of all possible atom pairs
...
# Define a matrix of all epsilons of all possible atom pairs
def getLennartJonesCrossEpsilon(allTypes, allMoleculeNumbers):
    ...
    # Fill the matrix, set to 0 if in the same molecule
    for i in range(0, allTypes.shape[0]):
        for j in range(0, allTypes.shape[0]):
...
def calculateForcesEnergy(atomListXYZNow, bondList, angleList, dihedralList, typeList, moleculeNumberList, boxSize, rLJCutOff, LJsigma, LJepsilon, LJsigma6, LJsigma12): # add dihedralList
    ...
    for a in range(0, angleList.shape[0]):
        ...
        # Get the correct forces: direction and magnitude
    ...
    pairwiseDistances = np.zeros([atomListXYZNow.shape[0], atomListXYZNow.shape[0]])
    
    # Fill pairwise distances
    for i in range(0, pairwiseDistances.shape[0]):
        pairwiseDistances[i,(i+1):] = floorDistanceVector(atomListXYZNow[i, :], atomListXYZNow[(i+1):, :], boxSize)
        
    
    # Set to zero if epsilon is zero
    ...
    # Create an list of all Van der Waals interactions
    ...
    # Print how many Van der Waals interactions
    print(LJList.shape[0])
    
    # Start timer
    codeTimer = time.time()
    
    # Define vectors atom1 and atom2:
    atom1 = LJList[:, 0]
    atom2 = LJList[:, 1]
    ...
    # Put the freely moving atoms back in the box
    ...
    # Adjust direction if boundary condition have to be taken into account:
...
# Get special distance, take into account box 
def floorDistanceVector(a, b, size):#(a, b, size):
